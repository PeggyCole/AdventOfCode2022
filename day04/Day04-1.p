...
procedure processData:
  define variable viTotalFullyCovered as integer   no-undo.
  
  for each ttCouple no-lock :
     if ( ttCouple.FromSection1 >= ttCouple.FromSection2 
        and ttCouple.ToSection1 <= ttCouple.ToSection2)
        or
        ( ttCouple.FromSection1 <= ttCouple.FromSection2 
        and ttCouple.ToSection1 >= ttCouple.ToSection2)
        
        then
        viTotalFullyCovered = viTotalFullyCovered + 1.
                
  end.
  
  message viTotalFullyCovered skip etime / 1000
